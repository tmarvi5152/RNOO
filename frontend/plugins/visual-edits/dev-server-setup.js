      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(
              `git -c user.name="visual-edit" -c user.email="edit@rpower.local" add "${targetFile}"`,
            );
            execSync(
              `git -c user.name="visual-edit" -c user.email="edit@rpower.local" commit -m "visual_edit_${timestamp}"`,
            );
          } catch (gitError) {
