...
export const useCartStore = create(
  persist(
    (set, get) => ({
      ...
      addItem: (item, quantity, modifiers, specialInstructions, merchantId, merchantSlug) => {
        const state = get();
        ...
        const modifierTotal = modifiers?.reduce((sum, m) => sum + (m.price || 0), 0) || 0;
        ...
        set(state => ({
          items: state.items.map(item => {
            if (item.id === cartItemId) {
              const modifierTotal = item.modifiers?.reduce((sum, m) => sum + (m.price || 0), 0) || 0;
              return {
                ...item,
                quantity,
