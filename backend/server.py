...
@api_router.get("/shepherd/merchants/{shepherd_merchant_id}/boarding-profile")
async def get_shepherd_boarding_profile(
    shepherd_merchant_id: str,
    cg: Optional[str] = Query(default=None),
    core_store_id: Optional[str] = Query(default=None),
    user: dict = Depends(require_role([UserRole.SUPER_ADMIN, UserRole.RESELLER]))
):
    """
    Build an onboarding profile by combining Shepherd merchant details and
    RPOWER Core API store data. Core data is prioritized when available.
    """
    shepherd = get_shepherd_client()
    if not shepherd:
        raise HTTPException(status_code=503, detail="Shepherd API not configured")
    ...
    try:
        shepherd_merchant = await shepherd.get_merchant(shepherd_merchant_id)
    except Exception as e:
        logger.error(f"Failed to fetch Shepherd merchant {shepherd_merchant_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Shepherd API error: {str(e)}")
    ...
    license_data = shepherd_merchant.get("License") or shepherd_merchant.get("license") or {}
    shepherd_profile = {
        "name": _first_non_empty(
            license_data.get("Name"),
            license_data.get("name"),
            shepherd_merchant.get("Name"),
            shepherd_merchant.get("name"),
        ),
        "address_line1": _first_non_empty(license_data.get("Address1"), license_data.get("address1")),
        "address_line2": _first_non_empty(license_data.get("Address2"), license_data.get("address2")),
        "city": _first_non_empty(license_data.get("City"), license_data.get("city")),
        "state": _first_non_empty(license_data.get("State"), license_data.get("state")),
        "zip_code": _first_non_empty(license_data.get("Zip"), license_data.get("zip")),
        "phone": _first_non_empty(license_data.get("Phone"), license_data.get("phone")),
        "email": _first_non_empty(license_data.get("Email"), license_data.get("email")),
        "url": _first_non_empty(license_data.get("URL"), license_data.get("url")),
    }
    ...
    core_lookup = None
    requested_core_store_id = (core_store_id or "").strip() or None
    selected_core_store_id = None
    core_store_candidates: List[Dict[str, Any]] = []
    core_payload = None
    ...
    if core_client and core_client.token:
        try:
            if cg:
                core_lookup = "cg"
                core_payload = await core_client.get_store_by_cg(cg)
            else:
                serial = shepherd_merchant_id[:5] if shepherd_merchant_id else ""
                core_lookup = "serial"
                core_payload = await core_client.get_store_by_serial_number(serial)

            if core_payload and "error" not in core_payload:
                stores = core_payload if isinstance(core_payload, list) else core_payload.get("stores", [])
                if isinstance(stores, list) and stores:
                    for store in stores:
                        guest_checks = store.get("guest_checks") or []
                        gc = guest_checks[0] if guest_checks else {}
                        core_store_candidates.append({
                            "store_id": _first_non_empty(store.get("store_id")),
                            "site_code": _first_non_empty(store.get("site_code")),
                            "name": _first_non_empty(store.get("name"), store.get("Name"), store.get("tag_name")),
                            "city": _first_non_empty(gc.get("city"), gc.get("City")),
                            "state": _first_non_empty(gc.get("state"), gc.get("State")),
                            "phone": _first_non_empty(gc.get("phone"), gc.get("Phone")),
                        })

                    matched_store = None

                    if requested_core_store_id:
                        for store in stores:
                            candidate_store_id = str(store.get("store_id") or "").strip()
                            candidate_site_code = str(store.get("site_code") or "").strip()
                            if requested_core_store_id == candidate_store_id or requested_core_store_id == candidate_site_code:
                                matched_store = store
                                break
                    ...
                    for store in stores:
                        if matched_store:
                            break
                        site_code = str(store.get("site_code") or "").strip()
                        if site_code and site_code == shepherd_merchant_id:
                            matched_store = store
                            break
                    if not matched_store:
                        matched_store = stores[0]
                    ...
                    if matched_store:
                        selected_core_store_id = _first_non_empty(matched_store.get("store_id"), matched_store.get("site_code"))
                        guest_checks = matched_store.get("guest_checks") or []
                        gc = guest_checks[0] if guest_checks else {}

                        core_profile = {
                            "name": _first_non_empty(matched_store.get("name"), matched_store.get("Name"), matched_store.get("tag_name")),
                            "address_line1": _first_non_empty(gc.get("address"), gc.get("Address")),
                            "address_line2": _first_non_empty(gc.get("address2"), gc.get("Address2")),
                            "city": _first_non_empty(gc.get("city"), gc.get("City")),
                            "state": _first_non_empty(gc.get("state"), gc.get("State")),
                            "zip_code": _first_non_empty(gc.get("zip"), gc.get("Zip")),
                            "phone": _first_non_empty(gc.get("phone"), gc.get("Phone")),
                            "email": None,
                            "url": _first_non_empty(gc.get("url"), gc.get("Url")),
                        }
        except Exception as e:
            logger.warning(f"Failed to fetch Core API profile for {shepherd_merchant_id}: {e}")
    ...
    for field in [
        ...
        if core_profile.get(field):
            merged[field] = core_profile[field]
            source[field] = "core"
        elif shepherd_profile.get(field):
            merged[field] = shepherd_profile[field]
            source[field] = "shepherd"
        else:
            merged[field] = None
            source[field] = "none"
    ...
    return {
        "shepherd_merchant_id": shepherd_merchant_id,
        "rpower_cg": cg,
        "requested_core_store_id": requested_core_store_id,
        "core_lookup": core_lookup,
        "core_store_id": selected_core_store_id,
        "core_store_candidates": core_store_candidates,
        "shepherd": shepherd_profile,
        "core": core_profile,
        "merged": merged,
        "source": source,
        "core_available": bool(core_client and core_client.token),
    }
